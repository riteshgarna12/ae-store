    ...
    <>
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <h2 style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: '1.4rem', color: 'var(--text-primary)' }}>🔍 Search Catalog</h2>
          <motion.button
        ...
        <div style={{ maxWidth: '650px', margin: '0 auto', width: '100%' }}>
          ...
          <div style={{ marginTop: '24px', maxHeight: '40vh', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {query.trim() !== '' && filtered.length === 0 ? (
              <p style={{ color: 'var(--text-muted)', textAlign: 'center', fontSize: '0.9rem', padding: '20px 0' }}>No packs matching your search</p>
            ) : (
              (query.trim() === '' ? bundles : filtered).map(b => (
                ...
                >
                  <div>
                    <h4 style={{ fontFamily: 'Outfit', fontSize: '0.95rem', color: 'var(--text-primary)', marginBottom: '2px' }}>{b.name}</h4>
                    <p style={{ color: 'var(--text-muted)', fontSize: '0.78rem' }}>{b.tagline}</p>
                  </div>
